    ...
    <nav className="fixed top-0 w-full bg-white/90 backdrop-blur-md border-b border-gray-100 z-50 shadow-sm">
      ...
      <div className="sm:hidden border-t border-gray-100 bg-gray-50/50 px-4 py-3 overflow-x-auto hide-scrollbar">
        <ul className="flex items-center gap-6 min-w-max">
          {navItems.map(({label, url, icon: Icon}) => (
            <li key={label}>
              <Link href={url} className="flex items-center gap-1.5 text-xs font-bold text-gray-600 hover:text-red-600 transition-colors">
          ...
          {session?.user && (
            <li>
              <button 
                onClick={() => signOut()} 
                className="flex items-center gap-1.5 text-xs font-bold text-gray-600 hover:text-red-600 transition-colors focus:outline-none"
              >
